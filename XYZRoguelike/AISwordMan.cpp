...
{
	AISwordMan::AISwordMan(std::shared_ptr<XYZEngine::GameObject> gameObject) : XYZEngine::AIMovementComponent(gameObject)
	{
	...
	void AISwordMan::Update(float deltaTime)
	{
		...
		if(!transformTarget->GetGameObject())
		{
			return;
		}
	...
	void AISwordMan::SetTarget(std::shared_ptr<XYZEngine::GameObject> gameObject)
	{
