...
void DeveloperLevel::Start() {
    ...
    auto enemySpawner =
        std::make_shared<NPCSpawner<Enemy<AICreeper, Explosion>>>(
            XYZEngine::Vector2Df{400.f, 400.f}, 1.f, 3, true, "EnemyCreeper");
    auto player = std::make_shared<Player>(std::forward<XYZEngine::Vector2Df>(
        {(x_Offset + width) / 2 * 64.f, (y_Offset + height) / 2 * 64.f}));
    auto swordMan = std::make_shared<Enemy<AISwordMan, Sword>>(
        XYZEngine::Vector2Df({350.f, 350.f}), "SwordMan");
