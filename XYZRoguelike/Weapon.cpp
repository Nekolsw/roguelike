...
namespace XYZEngine
{
	Weapon::Weapon(std::shared_ptr<GameObject> gameObject) : Component(gameObject)
	{
	...
	std::shared_ptr<GameObject> Weapon::GetGameObject()
	{
		return gameObject;
	}
	...
	{
		std::shared_ptr<CharacterStats> characterStats = collision.first->GetGameObject()->GetComponent<CharacterStats>();
		try {
			if (!characterStats) {
